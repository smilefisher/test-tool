...
export default function EditConnectionPage() {
  ...
  useEffect(() => {
    if (params.id) {
      fetchConnection(params.id as string);
    }
  }, [params.id]);
  ...
  async function fetchConnection(id: string) {
    try {
      ...
      setDb(data.db !== null ? String(data.db) : '0');
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      const data: Record<string, string | number | null> = {
        ...
        db: dbType === 'redis' && db !== null ? parseInt(db) : null,
        username: dbType === 'redis' ? null : (username || null),
        password: dbType === 'mongodb' ? null : (password || null),
        database_name: dbType === 'mysql' ? (databaseName || null) : null,
        uri: dbType === 'mongodb' ? (uri || null) : null,
      };
  ...
  return (
    <div className="min-h-screen bg-slate-50">
      ...
      <main className="max-w-2xl mx-auto px-6 py-8">
        <h1 className="text-2xl font-bold text-slate-800 mb-8">编辑数据库连接</h1>
